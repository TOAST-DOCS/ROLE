...
1. 페이징을 위한 요청/응답 모델
   **[Pageable]**
   ...
   |--------------|----------------|----------|
   | page         | Integer| 페이지 번호         |
   | itemsPerPage | Integer | 목록 당 조회될 아이템 수 |
   | sort              | List&lt;String>      | 	데이터 정렬 기준     |

   **[Page]**

   | Key          | Type       | 	Description |
   |------------|------------|----------|
   | totalItems         | Integer    | 전체 개수    |
   | items | List&lt;T> | 조회된 목록     |
...
1. Model
   ...
   **[User]**
   ...
   |roleRelations| 	List&lt;UserRoleRelation> |	연관 역할|

   **[UserRoleRelation]**

   |Key| 	Type              | 	Description           |
      |--------------------|------------------------|---|
   |scopeId| 	String            | 	적용 대상 ID              |
   |roleId| 	String            | 	역할 ID                 |
...
3. User 조회
   ...
   **[GetUserRequest]**
   ...
   |----------------------|-----------------------------------------------------|---|
   | userId               | String | 	사용자 ID                                             |
   | searchRoleOptionCode | SearchRoleOptionCode    | 	연관관계 권한을 포함해서 검색할 지 여부: DIRECT_ROLE, INDIRECT_ROLE |
   | roleIds              | List&lt;String> | 	역할 ID 목록      |
   | scopeIds             | List&lt;String> | 	적용 대상 ID 목록 |
...
4. User 목록 조회
   ...
   ⚠️ 요청/응답 시 사용되는 모델은 `Common`, `3. User 조회` 참고
...
7. User 역할 변경 내역 리스트 조회
   ...
   **[GetUserRoleHistoriesRequest]**
   ...
   | roleId              | String | 	역할 ID       |
   | scopeId             | String	| 적용 대상 ID
   | fromDateTime | OffsetDateTime   | 	변경 시작일시     |
   | toDateTime | OffsetDateTime   | 	변경 종료일시     |
    ...
    **[UserRoleHistory]**
    ...
    | Key                  | 	Type          | 	Description |
          |----------------|--------------|---|
    | userHistorySeq               | long           | 	순번           |
    | userId               | String         | 	사용자 ID      |
    | roleId              | String         | 	역할 ID       |
    | scopeId             | String	        | 적용 대상 ID
    | roleApplyPolicyCode | RoleApplyPolicyCode | 	역할 사용 여부: ALLOW, DENY     |
    | conditions | List&lt;ConditionBundle> | 	역할 조건 속성     |
    | command | UserRoleHistoryCommandCode | 	명령     |
    | executionTime | OffsetDateTime | 	변경 일시     |
...
4. Resource 목록 조회
   ...
   **[GetResourcesRequest]**
   ...
   | searchRoleOptionCode | 	SearchRoleOptionCode | 	역할 검색 시 하위 역할 포함 여부      |
...
1. 리소스 계층 구조 조회
   **[GetResourceHierarchyRequest]**
   ...
   | Key             | Type            | Description |
      |-----------------|-----------------|-------------|
   | userIds         | List&lt;String> | 사용자 ID 목록   |
   | roleIds         | List&lt;String> | 역할 ID 목록    |
   | operationIds    | List&lt;String> | 오퍼레이션 ID 목록 |
   | scopeIds        | List&lt;String> | 범위 ID 목록    |
   | resourceIds     | List&lt;String> | 리소스 ID 목록   |
   | resourcePath    | String          | 리소스 경로      |
   | resourceUiPath  | String          | 리소스 UI 경로   |
   ...
   **[ResourceHierarchy]**
   ...
   | name        | String                     | 리소스 이름                                  |
   | path        | String                     | 리소스 경로                                  |
   | uiPath      | String                     | 리소스 UI 경로<br/>계층 구조가 이 경로를 기반으로 만들어 진다  |
   | priority    | Integer                    | 우선 순위                                   |
   | resources   | List&lt;ResourceHierarchy> | 하위 리소스들                                 |
...
1. 특정 리소스의 인가 결과 확인
   ...
   **[GetResourceAuthorizationRequest]**
   ...
   | Key           | 	Type                           | Description                                               |
       |---------------|---------------------------------|-----------------------------------------------------------|
   | authRequestId | String                          | 사용자가 정의한 ID 값<br/>어떤 인증 조건에 대한 응답인지에 대해 명확하게 알아야 하는 경우 사용 |
   | operationId   | String                          | 오퍼레이션 ID                                              |
   | resourceId    | String                          | 리소스 ID                                                    |
   | resourcePath  | String                          | 리소스 경로                                                    |
   | scopeId       | String                          | 범위 ID                                                  |
   | attributes    | List&lt;AuthorizationAttribute> | 조건 속성 목록                                                     |
...
2. 여러 리소스의 인가 결과 확인
   ...
   **[GetResourceAuthorizationResponse]**
   ...
   | Key           | 	Type                           | Description                                  |
   |---------------|---------------------------------|----------------------------------------------|
   | authRequestId | String                          | 사용자가 정의한 ID 값<br/>요청 시에 보낸 값이 그대로 반환된다       |
   | operationId   | String                          | 오퍼레이션 ID                                 |
   | resourceId    | String                          | 리소스 ID                                       |
   | resourcePath  | String                          | 리소스 경로                                       |
   | scopeId       | String                          | 범위 ID                                        |
   | permission    | Boolean                         | 인가 결과<br/><br/>true : 권한 있음<br/>false : 권한 없음 |
   | attributes    | List&lt;AuthorizationAttribute> | 조건 속성 목록                                     |
