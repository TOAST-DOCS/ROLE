...
⚠️ See `User` for models used when request
...
7. Lookup user role change history list
...
7. View a list of all attributes that can be set in the Role
...
try { 
   ...
   // When an error occurs, the changes stored in the session are emptied through the rollback function   session.rollback(); 
}
```
